# Rip-Boy 3000 for Chrome

A retro CRT terminal new tab page with a matching browser theme. Two pieces:

1. **`pipboy-theme/`** — Chrome theme. Tints the browser chrome (tabs, toolbar, frame, address bar) with phosphor green.
2. **`pipboy-newtab/`** — New tab override extension. Replaces Chrome's new tab page with a customizable CRT terminal featuring search, shortcut tiles, a live clock, and status bars.
...
5. Click **Load unpacked** again and select the `pipboy-newtab` folder.
...
- **Customize branding**: three ways to access the settings:
  ...
  2. Go to `chrome://extensions`, find Rip-Boy New Tab, click **Details** → **Extension options**
...
**Change default shortcuts**: edit `DEFAULT_SHORTCUTS` at the top of `pipboy-newtab/newtab.js`.

**Change default brand text**: edit `DEFAULT_BRANDS` at the top of `pipboy-newtab/newtab.js`. These are what the CONFIG panel's **RESET DEFAULTS** button restores to.

**Change theme colors**: edit `pipboy-theme/manifest.json` — the `colors` block uses `[R, G, B]` values 0–255.

**Add a new phosphor color scheme**: in `pipboy-newtab/newtab.css`, copy one of the `:root[data-scheme="..."]` blocks, change the phosphor values, and add it to the `<select>` in `newtab.html` and the favicon hue table at the bottom of the CSS.
...
- **New tab**: `chrome://extensions` → find "Rip-Boy New Tab" → **Remove**.
